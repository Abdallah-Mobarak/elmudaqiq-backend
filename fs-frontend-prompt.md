...
- ⚠️ **نادِه مرة واحدة للإنشاء بس.** لو الفترة موجودة بالفعل، الـ endpoint **غير مدمّر** — بيرجّع الجدول المحفوظ زي ما هو (مش بيمسح ولا بيصفّر). عشان:
  - **إعادة فتح الشاشة** → استخدم `GET /comparative/:year` (مش POST).
  - **إضافة صف** → ضيفه في الجدول محلياً واعمل `PUT` (مش POST تاني).
