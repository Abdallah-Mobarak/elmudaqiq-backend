/*
  ...
  - You are about to alter the column `role` on the `contractstaff` table. The data in that column could be lost. The data in that column will be cast from `VarChar(191)` to `Enum(EnumId(5))`.
  - A unique constraint covering the columns `[contractId,userId,role]` on the table `ContractStaff` will be added. If there are existing duplicate values, this will fail.
...
-- DropIndex
DROP INDEX `ContractStaff_contractId_userId_key` ON `contractstaff`;
...
-- AlterTable
ALTER TABLE `contractstaff` MODIFY `role` ENUM('ASSISTANT_TECHNICAL_AUDITOR', 'FIELD_AUDITOR', 'CONTACT_PERSON') NOT NULL;

-- CreateIndex
CREATE UNIQUE INDEX `ContractStaff_contractId_userId_role_key` ON `ContractStaff`(`contractId`, `userId`, `role`);
