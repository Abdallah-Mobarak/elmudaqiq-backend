...
(async () => {
  const { contract, model, position, income } = await svc.generateFullReport(contractId);
  ...
  const html = renderFullReport({ contract, model, position, income, opinionType: "UNQUALIFIED" });
